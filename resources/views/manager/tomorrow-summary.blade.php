...
$today = Carbon\Carbon::parse('tomorrow');

@endphp

@section('content')

<h2>Visão dos agendamentos de amanhã ({{$today->format('d/m/Y')}})</h2>
<form action="{{route('manager.invoice')}}">
    @csrf
    <div class="card">
        <div class="card-header">Serviços agendados para amanhã <button type="submit" class="btn btn-sm btn-outline-primary float-right"><i class="fas fa-receipt"></i> Nota</button> </div>
        <div class="card-body">
